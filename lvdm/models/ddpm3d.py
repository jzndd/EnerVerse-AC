...
class ACWMLatentDiffusion(LatentDiffusion):
    ...
    @torch.no_grad()
    def get_traj_for_rlinf(self, sample_size, pose, w2c, c2w, intrinsic, radius=50):
        """
        this function takes camera info. and eef. poses as inputs, and outputs the trajectory maps.
        output traj map shape: (c, t, h, w)
        """        
    ...
    def inference(
        ...
        **kwargs
        ...
        if saving_video:
            ##
            video_dir = target_dir if video_dir is None else video_dir
        ...
        traj_samples_video = traj_samples_video[:n_valid]
        traj_samples_video = traj_samples_video.data.cpu().numpy()
        # for frame_i in range(traj_samples_video.shape[0]):
        #     frame = traj_samples_video[frame_i]
        #     cv2.imwrite(os.path.join(target_dir, "traj_{:05d}.jpg".format(frame_i)), frame[:,:,::-1])
